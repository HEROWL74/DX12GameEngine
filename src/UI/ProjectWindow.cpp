...
namespace Engine::UI
{
	...
	void ProjectWindow::setProjectPath(const std::string& path) noexcept
	{
		m_projectPath = path;
		refreshAssets();
	}
	...
	void ProjectWindow::refreshAssets()
	{
		...
		try
		{
			for (const auto& entry : std::filesystem::directory_iterator(m_projectPath))
			{
				...
				asset.extention = entry.path().extension().string();
			...
			std::sort(m_assets.begin(), m_assets.end(),
				[](const AssetInfo& a, const AssetInfo& b) {
					if (a.type != b.type)
					{
						return a.type < b.type; //フォルダを先にソート
					}
	...
	void ProjectWindow::drawAssetList()
	{
		if (ImGui::BeginTable("AssetTable", 3, ImGuiTableFlags_Resizable | ImGuiTableFlags_Sortable))
		{
			...
			for (auto& asset : m_assets)
			{
				...
				switch (asset.type)
				{
				case AssetInfo::Type::Folder: typeStr = "Folder";
				case AssetInfo::Type::Texture: typeStr = "Texture";
				case AssetInfo::Type::Material: typeStr = "Material";
				case AssetInfo::Type::Shader: typeStr = "Shader";
				}
